	...
	var buildTerritoryArray = function(collection){
		sayWhat('building array of territories; row: ', gameStats.get('rows'), 'columns: ', collection.get('columns'))
	...
	window.GameStats = Backbone.Model.extend({
		...
		initialize:function(turn){
		  ...
		  sayWhat(this.get('turn'))
		  sayWhat(this.toJSON())
		},

		increaseTurnCounter: function(){
			sayWhat('increasing turn')
			sayWhat(this.get('turn'))
	...
	window.Nation = Backbone.Model.extend({

		growOnce: function(){
			var nation = this.toJSON();
			sayWhat('before', nation);

			nation.stats.population = ~~(logisticGrowth(nation.stats.population,nation.stats.growthRate, nation.stats.carryingCapacity));
			this.set(nation);
			sayWhat('Triggered increase', this.get('stats'));
			this.trigger('change:stats')
			return nation.stats.population;

		}		

	});

	window.Nations = Backbone.Collection.extend({
		model: Nation,
		url: '/nations',

		increasePop: function(){

			var nations = this.models;
...
*	*	*	*	*	*	*	*/
	...
	$(document).ready(function() {
		...
		window.NationView = Backbone.View.extend({
			tagName:  'li',
			className: 'nation',
			...
			initialize: function() {
				...
				this.collection.bind('reset', this.render);
				this.template = _.template($('#nation-template').html())
			},
		...
		window.LibraryNationView = NationView.extend({});
		...
		window.LibraryView = Backbone.View.extend({
			...
			render: function(){
				...
				this.collection.each(function(nation) {
					var view = new LibraryNationView({
						model: nation,
						collection: collection
					});
					$nations.append(view.render().el)
				});
